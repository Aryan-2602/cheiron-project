...
Clicking any bar, point, node, or edge shows that datum's citations — NCT id, the exact supporting field value, and a link to the trial. On a merged network node it also lists the RxNorm surface forms folded into it, so a click on `pembrolizumab` shows both the Keytruda-named and Pembrolizumab-named trials behind one number. `meta.query_interpretation`, `meta.assumptions` and every `meta.warnings` entry render below the chart, since the disclosures are part of what makes a figure trustworthy — as does `meta.api_urls`, so any number on screen can be reproduced by hand against ClinicalTrials.gov rather than taken on trust.
...
| `statuses` | **Read from the query text directly**, mapped to the live-verified `overallStatus` vocabulary via a synonym table ("stopped early" → `TERMINATED`). Longer phrases match first, so `"not yet recruiting"` cannot also register as `RECRUITING` — they are opposite filters. Every phrase in the table has to mean a status and nothing else: bare `available`, `complete`, and `stopped` were removed because they are ordinary English first, and matching them turned "which countries have **available** melanoma trials" into an expanded-access filter. A status named *negatively* is recorded as an exclusion, never as the positive filter (see [status negation](#status-negation-is-detected-but-not-filterable)). |
...
| `total_studies_fetched` | Trials retrieved upstream, before any client-side filter. Equal to `total_studies_processed` unless a filter the API cannot express safely ran locally — and the gap between them is what says whether an empty chart describes the registry or only this sample. |
...
| `empty_reason` | `null` for a populated chart. `NO_MATCHING_TRIALS` when the search found nothing, `NO_CHARTABLE_DATA` when trials matched but the analysis produced no rows, `NO_MATCHES_IN_FETCHED_SAMPLE` when a capped fetch was filtered to zero locally — three different answers, and a caller may handle them differently. |

### Errors

| Status | Code | When |
|---|---|---|
| 200 | — | Success, **including three kinds of legitimate emptiness**, distinguished by `meta.empty_reason`. `NO_MATCHING_TRIALS`: the search found nothing. `NO_CHARTABLE_DATA`: trials matched but the requested analysis produced no rows — every trial missing a start date, or no drug pair meeting the co-occurrence threshold. `NO_MATCHES_IN_FETCHED_SAMPLE`: the search matched trials, the fetch stopped at its cap before reading them all, and a local filter then removed every one that was read — so the answer may sit in the pages never fetched, and the advice is to raise `max_studies` rather than to broaden the query. All three return an empty `data` array with the same metadata a populated chart carries, and a warning naming the cause and the matched count. None is a fault. |
...
| 500 | `INTERNAL_ERROR` | Anything the handlers above did not anticipate. The `message` is fixed text and carries nothing from the exception — an error string can hold a fragment of the query or an upstream URL, and this is the one response produced by a code path nobody reasoned about. The detail is logged against the request id instead. |
...
**Status words are matched on whole-word boundaries.** A raw substring search read `"unavailable"` as `AVAILABLE` and `"incomplete"` as `COMPLETED`, inventing filters the question never asked for. The same class covered `uncompleted`, `preterminated`, and `reenrolling`. A hyphen is a word boundary too, so `"re-enrolling"` matched `enrolling` until a status phrase sitting at the tail of an unrelated compound was rejected outright — `"non-recruiting"` and `"no-longer-recruiting"` still read as exclusions, because a hyphenated *cue* is the construction the negation scan exists to read. Both the query builder and the client-side filter now normalise statuses through one shared function, so a padded or oddly-cased value cannot pass the first check and fail the second — which previously meant no status filter applied anywhere, silently.

<a id="status-negation-is-detected-but-not-filterable"></a>
**Status negation is applied locally, never as an upstream enumeration.** `"trials that are not recruiting"` used to send `aggFilters=status:rec` upstream, returning precisely the opposite set, because `"recruiting"` matched as a substring. A negation cue before a status now records it as an *exclusion*, and the exclusion is enforced against the fetched records.

It is deliberately **not** turned into an upstream filter by enumerating the other statuses: a live 1,000-trial sample returned `APPROVED_FOR_MARKETING`, which is not in the synonym table at all, so an enumerated "everything except X" would silently drop those trials. Filtering locally keeps the predicate negative — a status outside the vocabulary is kept, which is what the question asked for — and the narrowing is disclosed in `meta.warnings`. Because the exclusion counts as a predicate, such a question is retrieved by its filters rather than falling through to the free-text fallback.

A negation whose complement is a **list** governs the whole list: `"neither recruiting nor completed"` and `"excluding recruiting and completed"` exclude both. `"not"` is treated differently on purpose — it is clausal, so `"not recruiting and completed"` is genuinely ambiguous and keeps the narrow reading, excluding only `RECRUITING`. Excluding less than asked shows up on the chart; excluding more does not.
