...
EVIDENCE: dict[str, tuple[str, Callable[[dict[str, Any]], Any]]] = {
    ...
    # The graph builder's own extractor, so a drug node's evidence quotes
    # exactly the interventions that could have produced it. Filtering by type
    # alone was not enough: "Placebo for lenvatinib" is typed DRUG, so it
    # passed the type check and buried the real agent among entries the node
    # was never built from. extract_drugs also applies the stopword and
    # normalization rules, which is what makes the two agree.
    "drug": (
        "armsInterventionsModule.interventions[].name",
        lambda r: [label for _key, label in extract_drugs(r)],
    ),
}
